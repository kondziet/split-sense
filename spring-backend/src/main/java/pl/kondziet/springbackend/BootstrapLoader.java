...
public class BootstrapLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        List<Group> groups = groupPersistencePort.loadUserGroups(new UserId(savedUser1.getId()));

        groupMembershipPersistencePort.saveGroupMembership(new GroupMembership(new UserId(savedUser2.getId()), groups.stream().findFirst().get().getId()));
